...
{
    SQLite::Statement stmt(*this->database_,
                           R"sql(SELECT Value FROM translation WHERE Key=$Key AND Locale=$Locale;)sql");
    stmt.bind("$Key", key);
...
void packer::Packer::insert_log(const std::string &level, const std::string &logger, const std::string &message) const
{
    SQLite::Statement stmt(
        *this->database_,
        R"sql(INSERT INTO Log (Level, Date, Logger, Message) VALUES ($Level, $Date, $Logger, $Message);)sql");
...
std::vector<packer::LogEntry> packer::Packer::get_latest_logs(const int &limit) const
{
    ...
    SQLite::Statement stmt(
        *this->database_,
        R"sql(SELECT Level, Date, Logger, Message FROM Log ORDER BY Date DESC, Id ASC LIMIT $Limit;)sql");
    stmt.bind("$Limit", limit);

    while (stmt.executeStep())
    {
        collection.emplace_back(packer::LogEntry(stmt.getColumn(0).getText(), stmt.getColumn(1).getInt64(),
                                                 stmt.getColumn(2).getText(), stmt.getColumn(3).getText()));
    }
...
std::vector<packer::Toggle> packer::Packer::get_toggles() const
{
    ...
    while (stmt.executeStep())
    {
        collection.emplace_back(packer::Toggle(stmt.getColumn(0).getText(), stmt.getColumn(1).getInt() == 1));
    }
    return collection;
}
